      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (Array.isArray(details.participants) && details.participants.length > 0) {
          participantsHTML = `<ul class="participants-list">${details.participants
            .map((p) => `<li><span class="participant-badge">${escapeHTML(p)}</span><button class="participant-remove" data-activity="${escapeHTML(name)}" data-email="${escapeHTML(p)}" aria-label="Remove ${escapeHTML(p)}">✖</button></li>`)
            .join("")}</ul>`;
        } else {
